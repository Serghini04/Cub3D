...
/*   Updated: 2024/09/17 12:32:54 by hidriouc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_previous(char **arr, int i, int j)
{
	while (1)
	{
		...
		{
			if (ft_strlen(arr[i - 1]) - 1 < j || is_sp(&arr[i - 1][j], 0) || \
				is_sp(arr[i - 1], j + 1))
				return (1);
			if (arr[i - 1][j] == ' ')
			{
...
{
	if (ft_strlen(arr[i - 1]) - 1 > j && arr[i - 1][j] == ' ' && \
